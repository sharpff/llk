...
static void inner_set_ap_info(const ap_passport_t *passport);
...
{   
    ...
    if (frame->frame_type == BEACON) {
        ...
        if (!gin_airconfig_channel_locked)
        {
            ...
            if (ret) {
                APPLOG("ssid[%s] passwd[%s] base[%d]\r\n", passport.ssid, passport.passwd, base);
                airconfig_stop();
...
static void inner_set_ap_info(const ap_passport_t *passport) {
    ...
    if (strlen(passport->passwd))
        gin_sta_net.security.type = WLAN_SECURITY_WPA2;
    else
        gin_sta_net.security.type = WLAN_SECURITY_NONE;
    /* Set the passphrase */
    strncpy(gin_sta_net.security.psk, passport->passwd, strlen(passport->passwd));
...
static void thread_lelink_proc(os_thread_arg_t thandle) {
    ...
    void *ctx_r2r = (void *)nwNew(REMOTE_IP, REMOTE_PORT, 0, 0);
    void *ctx_q2a = (void *)nwNew(NULL, 0, NW_SELF_PORT, ctx_r2r);
    // int i, ret = 0;

    while (1) {
        pollingState(1000, ctx_r2r, ctx_q2a);
        doPollingQ2A(ctx_q2a);
