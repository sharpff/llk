...
#define MAX_INFO_LEN (64/4) + 1 // 32 + 32 + 1(rand) bytes for passwd
enum {
...
static uint8_t gin_passwd_len;
static uint8_t gin_passwd_crc;
...
int airconfig_get_info(int len, int base, ap_passport_t *passport, const char *currSSID, int currSSIDLen) {
    ...
    if (AIRCONFIG_STATE_GET_HEADER == gin_airconfig_state) {
        ...
        case 0x03: { // low 4 bits of ssid crc
            ...
            // magic part end
            // prefix part start
        case 0x04: { // high 4 bits of passwd len
            gin_airconfig_flag_header[hdata]++;
            if (MAX_COUNT_HEADER > gin_airconfig_flag_header[hdata] || MAX_COUNT_HEADER < gin_airconfig_flag_header[hdata]) 
                break;
            gin_passwd_len |= (ldata << 4);
            // gin_airconfig_flag_header[hdata] = 1;
            LELOG("0x04 flag_header[%d] [0x%x] \r\n", gin_airconfig_flag_header[hdata], gin_ssid_len);
        }break;
        case 0x05: { // low 4 bits of passwd len
            gin_airconfig_flag_header[hdata]++;
            if (MAX_COUNT_HEADER > gin_airconfig_flag_header[hdata] || MAX_COUNT_HEADER < gin_airconfig_flag_header[hdata]) 
                break;
            gin_passwd_len |= ldata;
            // gin_airconfig_flag_header[hdata] = 1;
            LELOG("0x05 flag_header[%d] [0x%x] \r\n", gin_airconfig_flag_header[hdata], gin_ssid_len);
        }break;
        case 0x06: { // high 4 bits of passwd crc
            gin_airconfig_flag_header[hdata]++;
            if (MAX_COUNT_HEADER > gin_airconfig_flag_header[hdata] || MAX_COUNT_HEADER < gin_airconfig_flag_header[hdata]) 
                break;
            gin_passwd_crc |= (ldata << 4);
            // gin_airconfig_flag_header[hdata] = 1;
            LELOG("0x06 flag_header[%d] [0x%x] \r\n", gin_airconfig_flag_header[hdata], gin_ssid_len);
        }break;
        case 0x07: { // low 4 bits of passwd crc
            gin_airconfig_flag_header[hdata]++;
            if (MAX_COUNT_HEADER > gin_airconfig_flag_header[hdata] || MAX_COUNT_HEADER < gin_airconfig_flag_header[hdata]) 
                break;
            gin_passwd_crc |= ldata;
            // gin_airconfig_flag_header[hdata] = 1;
        ...
        if ((isHeaderReady())) {
            ...
            // TODO:
            gin_spencial_total = gin_passwd_len + gin_ssid_len + 1;
            // gin_spencial_total = gin_ssid_len;
            gin_info_block_remain = gin_info_block_total = (gin_spencial_total - 1)/4 + 1;
            // gin_one_remain = gin_passwd_len%4 ? gin_passwd_len%4 : 4;
            #ifdef AIRCONFIG_DEBUG_LEVEL1
            LELOG("DONE AIRCONFIG_STATE_GET_HEADER ssid[0x%02x][%d] passwd[0x%02x][%d] remain block[%d] \r\n", 
                gin_ssid_crc, gin_ssid_len, gin_passwd_crc, gin_passwd_len, gin_info_block_remain);
            #endif
        }
    }
        ...
        if (currSSID && 0 < currSSIDLen) {
            ...
            if (tmpCRC8 == gin_ssid_crc) {
                ...
                int ssidIdx = gin_passwd_len%4 + 1;
                int i, j, k = 0;
                int startBlockIdx = gin_passwd_len/4;
                ...
                if (gin_passwd_len%4) {
                    gin_spencial_total = gin_passwd_len + 1 + (gin_ssid_len >= (4 - ssidIdx) ? (4 - ssidIdx) : gin_ssid_len);
                } else {
                    gin_spencial_total = gin_passwd_len;
                }
                gin_info_block_remain = (gin_spencial_total - 1)/4 + 1;
                // gin_one_remain = gin_passwd_len%4 ? gin_passwd_len%4 : 4;
                LELOG("DONE PRE SSID [%s] ssid[0x%02x][%d] passwd[0x%02x][%d] remain block[%d] total[%d]\r\n", currSSID,
                    gin_ssid_crc, gin_ssid_len, gin_passwd_crc, gin_passwd_len, gin_info_block_remain, gin_spencial_total);
                gin_ssid_crc = 0;
        ...
        if (!gin_info_block_remain) {
            ...
            LELOG("DONE ok gin_info_block_total[%d] gin_passwd_len[%d] gin_ssid_len[%d] last_bytes[%d]\r\n", 
                gin_info_block_total, gin_passwd_len, gin_ssid_len, last_bytes);

            for (i = 0; i < gin_info_block_total; i++) {
                int bytes = ((i+1) == gin_info_block_total) ? last_bytes : 4;
                for (j = 0; j < bytes; j++) {
                    if (i*4 + j < gin_passwd_len) {
                        passport->passwd[m++] = gin_info[i].data[j];
                    } else if (i*4 + j > gin_passwd_len) {
