    ...
    void *funcFindToken)
{
    ...
    if (commonHeader->encsum != crc8(cmdHeader, nwLen - sizeof(CommonHeader))) {
    ...
    case ENC_TYPE_STRATEGY_0: {
            ...
            if (cmdHeader->encsum != crc8(payloadHeader, payloadLen)) {
            // if (cmdHeader->encsum != crc8(payloadHeader, sizeof(PayloadHeader) + payloadHeader->len)) {
    ...
    case ENC_TYPE_STRATEGY_13: {
            ...
            if (ENC_TYPE_STRATEGY_11 == commonHeader->encType) {
                ...
                // }
                if (funcFindToken) { // it must be local & RemoteRsp
    ...
    case ENC_TYPE_STRATEGY_233: {
            ...
            cmdHeader = (CmdHeader *)(tmpBuf + sizeof(CommonHeader));
            if (cmdHeader->encsum != crc8(tmpBuf + sizeof(CommonHeader) + encLenCmdHeader, encLenPayload)) {
            // if (cmdHeader->encsum != crc8(payloadHeader, sizeof(PayloadHeader) + payloadHeader->len)) {
                LELOGW("LELINK_ERR_CHECKSUM_2ND_ERR [%d] \r\n", LELINK_ERR_CHECKSUM_2ND_ERR);
                return LELINK_ERR_CHECKSUM_2ND_ERR;
            }
...
int lelinkPack(void *ctx, 
    ...
    switch (encType) {
        case ENC_TYPE_STRATEGY_0: {
            ...
            cmdHeader->encsum = crc8(payloadHeader, sizeof(PayloadHeader) + pbLen);
            ...
            getTerminalUUID(cmdHeader->uuid);
            ...
            commonHeader->encsum = crc8(cmdHeader, sizeof(CmdHeader) + sizeof(PayloadHeader) + pbLen);
        }break;
        case ENC_TYPE_STRATEGY_11:
        case ENC_TYPE_STRATEGY_13: {
            uint8_t iv[AES_LEN] = { 0 };
