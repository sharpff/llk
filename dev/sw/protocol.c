...
static CmdRecord tblCmdType[] = {
    ...
    { LELINK_CMD_CLOUD_IND_OTA_REQ, LELINK_SUBCMD_CLOUD_IND_OTA_REQ, NULL, cbCloudIndOTARemoteReq },
    { LELINK_CMD_CLOUD_IND_OTA_RSP, LELINK_SUBCMD_CLOUD_IND_OTA_RSP, NULL, cbCloudIndOTALocalRsp},
...
void testSengine() {
    ...
    ret = sengineCall((const char *)ginScriptCfg.data.script, ginScriptCfg.data.size, 
        "cvtStd2Pri", (const uint8_t *)json, strlen(json), buf, sizeof(buf));
    ...
    buf[3] = 0xef;
    ret = sengineCall((const char *)ginScriptCfg.data.script, ginScriptCfg.data.size, 
        "cvtPri2Std", buf, 4, (uint8_t *)json, sizeof(json));
...
int lelinkInit(void *auth) {
    ...
    if (NULL == auth) {
     ...
     setOriRemoteServer(((AuthData *)(auth))->remote, strlen(((AuthData *)(auth))->remote), ((AuthData *)(auth))->port);
     getOriRemoteServer(((AuthData *)(auth))->remote, MAX_REMOTE, &(((AuthData *)(auth))->port));
...
static int cbCtrlCmdRemoteReq(void *ctx, const CmdHeaderInfo* cmdInfo, const uint8_t *dataIn, int dataLen) {
	...
	getTerminalStatus((char *)data, sizeof(data));
...
static int cbCtrlGetStatusLocalRsp(void *ctx, const CmdHeaderInfo* cmdInfo, const uint8_t *data, int len, uint8_t *dataOut, int dataLen) {
    ...
    ret = getTerminalStatus((char *)status, sizeof(status));
...
static int cbCloudReportLocalReq(void *ctx, const CmdHeaderInfo* cmdInfo, uint8_t *dataOut, int dataLen) {
    ...
    sprintf(report, "{\"uuid\":\"%s\"}", uuid);
    ret = doPack(ctx, ENC_TYPE_STRATEGY_14, cmdInfo, (const uint8_t *)report, strlen(report), dataOut, dataLen);
...
static int cbCloudReportOTAQueryLocalReq(void *ctx, const CmdHeaderInfo* cmdInfo, uint8_t *dataOut, int dataLen) {
    ...
    sprintf(query, "{\"uuid\":\"%s\",\"fw\":\"%s\"}", uuid, ver);
    LELOG("%s\r\n", query);
