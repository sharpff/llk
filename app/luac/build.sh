...
if [ "$1" = "gdb" ]; then
	...
	do_copy "$MAIN_PATH/hal/luac/" "$LinuxGDB/dev/hal/luac/"
	do_copy "$MAIN_PATH/sw/" "$LinuxGDB/dev/sw/"
	cp "$MAIN_PATH/app/luac/main.c"  "$LinuxGDB/dev/app/luac"
	# cp "$MAIN_PATH/app/luac/0x1c3000.bin"  "$LinuxGDB/dev/app/luac/Debug"
	pushd $LinuxGDB/dev/app/luac/Debug > /dev/null 2>&1
	chmod 777 * -R
	popd > /dev/null 2>&1
