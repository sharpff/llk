...
void bl_print_internal(char *fmt, va_list ap)
{
    ...
    *bp = 0;
    for (bp = buf; *bp; bp++) {
        hal_uart_put_char(CONSOLE_UART,*bp);
    }
}
...
void hw_uart_init(void)
{
    ...
    hal_uart_init(CONSOLE_UART, &config);

#if 0
    hal_uart_put_char(CONSOLE_UART, 'a');
    hal_uart_put_char(CONSOLE_UART, 'b');
    hal_uart_put_char(CONSOLE_UART, 'c');
    hal_uart_put_char(CONSOLE_UART, '\n');
