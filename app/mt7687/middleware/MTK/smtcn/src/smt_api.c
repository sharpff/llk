...
/*int lelink_raw_pkt_handler(unsigned char *payload, unsigned int len)
{
    ...
    if (len != 0) {
        ...
        if (wifi_connection_parse_beacon(payload, len, &ap_data) >= 0) {
            if ((ap_data.ssid_length == saved_smtcn_info.ssid_len) &&
                    !os_memcmp(ap_data.ssid, saved_smtcn_info.ssid, ap_data.ssid_length)) {
                xTimerStop(scan_timer, tmr_nodelay);
#if (smtcn_debug == 1)
                ...
                os_memcpy(ssid, saved_smtcn_info.ssid, saved_smtcn_info.ssid_len);
                os_memcpy(passwd, saved_smtcn_info.pwd, saved_smtcn_info.pwd_len);
...
static int32_t smtcn_scan_connect_ap(void)
{
    ...
    if (sc_scan_init() != 0) { //WIFI_PORT_STA ,ssid&passwd 
    ...
    if(SCAN_FLAG_FIN == saved_smtcn_info.scan_flag)
	{
        sc_connect();//scan found Target AP,then to connect the Target AP;
    }
	else if(SCAN_FLAG_NOT_FOUND == saved_smtcn_info.scan_flag){
#if (smtcn_debug == 1)
        uint8_t ssid[WIFI_MAX_LENGTH_OF_SSID+1] = {0};
        os_memcpy(ssid, saved_smtcn_info.ssid, WIFI_MAX_LENGTH_OF_SSID);
...
int32_t smtcn_rx_handler(uint8_t *payload, uint32_t len)
{
    ...
    os_memcpy(pkt.payload, payload, SMTCN_STATIC_BUF_SIZE);
    ...
    os_memcpy(pkt.payload, payload, len);
...
int32_t mtk_scan_connect_ap(unsigned char *ssid, unsigned char *passwd)
{
    ...
    if(ssid != saved_smtcn_info.ssid){
        os_memcpy(saved_smtcn_info.ssid, ssid, strlen((const char*)ssid));
        saved_smtcn_info.ssid_len = strlen((const char*)ssid);
    }

    if(passwd != saved_smtcn_info.pwd){
        os_memcpy(saved_smtcn_info.pwd, passwd, strlen((const char*)passwd));
...
static void smtcn_rcv_pkt_thread(void *pvParameters)
{
    ...
    os_memset((void *)&saved_smtcn_info, 0, sizeof(saved_smtcn_info));
...
wifi_smart_connection_status_t wifi_smart_connection_init (const uint8_t *key, const uint8_t key_length, wifi_smart_connection_callback_t  callback)
{
    ...
    #elif (CFG_SUPPORT_SMNT_PROTO == 6)
        psmtcn_proto_ops = &lelink_proto_ops;
    #endif
    }
...
wifi_smart_connection_status_t wifi_smart_connection_start (uint16_t timeout_seconds)
{
    ...
    printf("wifi_smart_connection_start mode %d", origin_opmode);
	...
	//create task: while(1) recv packet from Queue;
    if (xTaskCreate(smtcn_rcv_pkt_thread, "smart connection thread", 1024, NULL, configTIMER_TASK_PRIORITY, &smtcn_handle) != pdPASS) {
    ...
    //recv packet from wifi driver, then send to Queue; 
    wifi_config_register_rx_handler((wifi_rx_handler_t)smtcn_rx_handler);

    os_memset((void *)&smt_conn_st, 0, sizeof(smt_conn_st));
    smt_conn_st.cur_chanl = channel_list[0];

    wifi_config_set_channel(WIFI_PORT_STA, smt_conn_st.cur_chanl);
#if (smtcn_debug == 1)
    os_memset(channel_times, 0, sizeof(channel_times));
    channel_times[smt_conn_st.cur_chanl - 1]++;
#endif
    xTimerStart(switch_channel_timer, tmr_nodelay);
...

{
    ...
    if(saved_smtcn_info.pwd_len > 0){
        if(password != NULL){
            os_memcpy(password, saved_smtcn_info.pwd, saved_smtcn_info.pwd_len);
        }
    ...
    if(saved_smtcn_info.tlv_data != NULL){
        if(tlv_data != NULL){
            os_memcpy(tlv_data, saved_smtcn_info.tlv_data, saved_smtcn_info.tlv_data_len); 
        }
