...
void leLedReset(void) {
    ...
    APPLOG("leLedReset");
    halFlashErase(NULL, CUST_BASE - GW_CONF_LENGTH, GW_CONF_LENGTH);
    halFlashWrite(NULL, (uint8_t *)&ledOriginData, sizeof(ledDevice_t), CUST_BASE - GW_CONF_LENGTH, 0);
}
...
static int leLedWriteConfigData(ledDevice_t* device) {
    ...
    ret = halFlashWrite(NULL, (uint8_t *)device, sizeof(ledDevice_t), CUST_BASE - GW_CONF_LENGTH, 0);
    return ret;
}

static int leLedReadConfigData(ledDevice_t* device) {
    int ret;
    ret = halFlashRead(NULL, (uint8_t*)device, sizeof(ledDevice_t), CUST_BASE - GW_CONF_LENGTH, 0);
