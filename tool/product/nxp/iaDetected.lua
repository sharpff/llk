...
function getBeingReservedInfo()
    -- CUSTOMIZATION START
    local tblInfo = {'10000100131000010011B01BD2F0003B-00124B0011E825F4'}
    -- CUSTOMIZATION END
    return tblInfo
end
...
function s2IsConditionOKExt(selfStatus, rmtStatus)
    ...
    -- if nil ~= string.find(rmtUUID, '10000100101000010007E81863C38E75') then
        if 10 ~= tblNewStatus["ias"] then
            tblConditions[retIdx] = 1
            print('ok 1\r\n')
        end
    -- else
        -- tblOldStatus = getOldStatus('10000100131000010011B01BD2F0003B-00124B0011E825F4')
        -- if tblOldStatus then
