...
typedef struct {
	...
	/** If TLS connection with specific certificates is to be done,
	    ...
	    tls_create_client_context(). If TLS connection without specific
 ...
 */
typedef int (*content_fetch_cb)(http_session_t handle,
...
typedef struct {
	 ...
	 * above is NULL.
	 */
	content_fetch_cb content_fetch_cb;
 ...
 * http_open_session().
 *
 * @return Returns the same SSL_CTX parameter from httpc_cfg passed in call
